...
All configuration steps can be done either using SAP BTP cockpit or the Cloud Foundry Command Line Interface \(cf CLI\).
...
Shipping logs from applications deployed on SAP BTP, Cloud Foundry environment can be configured using one of the below options:
...
2.  To bind the application via command line interface, execute the following command:
    ...
    > ### Note:  
    > Although the command line interface prompts you to restage the application, the binding takes effect without restaging.
...
### Share Service Instances Across Different Spaces
...
**Bind the Application to User Provided Service Using cf CLI**
...
5.  Create the User Provided Service \(using either basic auth or mTLS\):
